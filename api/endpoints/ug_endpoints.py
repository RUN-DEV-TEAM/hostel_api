...
async def allocate_room_to_student_in_session_func(mat_no:str, session: async_sessionmaker = Depends(get_session), user: ReturnSignUpUser =Depends(require_permission())):
  ...
  res = await student_service.get_student_profile_and_allocate_room_to_the_student_service(str(mat_no).strip(),user_meta,session)
  if not res[0]:
...
@router.get("/get_available_space_from_guest_house",response_model="")
